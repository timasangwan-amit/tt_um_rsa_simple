...
// Uses square-and-multiply with a pipelined shift-and-add multiplier.
// One bit per clock in modmul => fits in a TT 1x1 tile.
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_INIT: begin
                    res <= 32'd1; b <= {16'd0, base}; exp <= exponent;
                    state <= S_CHECK;
                end
                S_CHECK: begin
                    if (exp == 16'd0) begin
...
// modmul: (a * b) % m — shift-and-add, 32 cycles, no large multiplier.
module modmul (
    input  wire        clk,
    input  wire        rst,
    input  wire        start,
    ...
    reg [31:0] acc, base, bb;
    reg [5:0]  cnt;
    reg        active;

    wire [32:0] base_dbl  = {1'b0, base} + {1'b0, base};
    wire [31:0] base_next = (base_dbl >= {1'b0, m}) ? base_dbl[31:0] - m : base_dbl[31:0];
    wire [32:0] acc_add   = {1'b0, acc} + {1'b0, base};
    wire [31:0] acc_next  = (acc_add  >= {1'b0, m}) ? acc_add[31:0]  - m : acc_add[31:0];
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (start) begin
                acc <= 32'd0; base <= a % m; bb <= b; cnt <= 6'd0; active <= 1'b1;
            end else if (active) begin
                if (cnt == 6'd32) begin
                    result <= acc; done <= 1'b1; active <= 1'b0;
                end else begin
                    if (bb[0]) acc <= acc_next;
                    base <= base_next; bb <= bb >> 1; cnt <= cnt + 6'd1;
                end
            end
        end
    end
